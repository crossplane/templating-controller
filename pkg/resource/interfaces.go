...
// TemplatingEngine is used as main generation engine by the templating reconciler.
// Its input is typically a Custom Resource instance and output is various
// Kubernetes objects generated by the given implementation of the TemplatingEngine.
type TemplatingEngine interface {
	Run(ParentResource) ([]ChildResource, error)
}

// TemplatingEngineFunc used for supplying only one function as templating engine.
type TemplatingEngineFunc func(ParentResource) ([]ChildResource, error)

// Run calls the TemplatingEngineFunc function.
